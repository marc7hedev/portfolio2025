...
- **Image Optimization**: Configured `images.unoptimized = true` in `next.config.ts` to ensure compatibility with static export (Hostinger).
- **SEO**: Added `JsonLd` component for structured data and enhanced `metadata` in `layout.tsx` with OpenGraph and Twitter tags.
